...
async function createPaymentRequest(req, h) {
    ...
    prices.forEach((pairPrice) => {
        if (pairPrice.amount && Number(pairPrice.amount) > 0) {
            let amount = math.bignumber(0);
            const originalAmount = math.bignumber(pairPrice.amount);
            ...
            amount = math.divide(amount, 1e8);
            pairPrice.amount = amount.toString();
        }
    });
    // console.log('adjust', prices);
    const paymentRequest = new PaymentRequest({
        amount,
        currency,
        prices,
        fees,
        markup: config.markup, // included in prices already
        memo,
        ref,
        createdBy
    });
...
async function paymentProtocolHandler(h, paymentRequest, currCode) {
    ...
    if (paymentRequest.status !== 'new' && paymentRequest.status !== 'unpaid') {
        ...
        return h.response({error: `Payment request no longer payable: ${paymentRequest.status}`}).code(400);
    }
    try {
        payPro = await coins.assemblePaymentRequest(currCode, paymentRequest);
    } catch (e) {
        console.error(e.message, JSON.stringify(paymentRequest));
        return h.response({error: 'Internal error'}).code(500);
    }
