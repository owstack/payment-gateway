...
function getDecodedJWT(token, json) {
    try {
        const cert = json.keys.rsa[0];
        const buf = Buffer.from(cert, 'utf8');
        return jwt.verify(token, buf, {algorithms: ['RS512']});
    } catch (e) {
        console.error(e);
        return {};
    }
...
function identifyFromJWT(request) {
    ...
    const decoded = getDecodedJWT(token, request.server.app.identityServicePublicKey);
    return decoded.sub;
}
...
async function getConversionRates(currency, amount) {
    ...
    Object.keys(config.rates).forEach(async (pair) => {
        promises.push(new Promise(async (resolve, reject) => {
            ...
            try {
                const rateResponse = await request({url, json: true});
                return resolve({pair, amount, rate: rateResponse.rate});
            } catch (e) {
                reject(e);
            }
