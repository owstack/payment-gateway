...
coins.assemblePaymentRequest = async function (currencyCode, paymentRequest) {
    ...
    details.set('merchant_data', Buffer.from(paymentRequest.ref || '', 'utf8'));
...
const broadcastTx = function (currencyCode, serializedtx) {
    const uri = `${config.explorerAPIs[currencyCode]}/tx/send`;
    console.log('Broadcasting TX', 'POST', uri, serializedtx.toString('hex'));
    return request({
        method: 'POST',
        uri,
        body: {
...
coins.receivePayment = async function (req, currencyCode, payload, paymentRequest) {
    ...
    const refundArray = [];
    refundTo.forEach((refundOutput) => {
        const refundObject = {
            amount: refundOutput.get('amount').toString(),
            script: refundOutput.get('script').toString('hex')
        };
        refundArray.push(refundObject);
    });
    ...
    transactions.forEach((serialized) => {
        const tx = new coins[currencyCode].lib.Transaction(serialized.toString('hex'));
        paymentRequest.received.push({
            currency: currencyCode,
            txid: tx.id,
            merchantData: merchantData.toString('utf8'),
            refundTo: refundArray,
            memo
        });
    ...
    try {
        ...
        await req.server.app.mailService.sendMail(email);
    } catch (e) {
        console.error('Error processing a payment', e);
