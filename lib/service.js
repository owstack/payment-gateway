        ...
        Payment.watch({fullDocument: 'updateLookup'})
            .on('change', (data) => {
                if (data.operationType === 'update') {
                    ...
                    // console.log('Payment request updated', JSON.stringify(paymentRequestObject, null, 2));
                    this.server.publish(`/${data.documentKey._id}`, {status: paymentRequestObject.status});
                }
            });
